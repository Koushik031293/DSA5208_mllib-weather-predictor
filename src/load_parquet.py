...
# Read Parquet
df = spark.read.parquet("data/noaa_parquet/2024")
...
# # Distinct partitions
df.select("year","month").distinct().orderBy("year","month").show()

(df.repartition("year","month")
   .coalesce(8)  # tune per partition; try 4–16 depending on size
   .write.mode("overwrite")
   .partitionBy("year","month")
   .parquet("data/noaa_parquet/2024_compacted"))